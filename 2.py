...
def calculate():
    ...
    try:
        ...
        messagebox.showinfo('Обнаружен чёрт', 'Разрешается вводить только цифры!')
        calc.insert(0, 0)
    except ZeroDivisionError:
        messagebox.showinfo('Обнаружен чёрт', 'На ноль делить нельзя!')
        calc.insert(0, 0)
...
def make_clear_button(operation):
    return tk.Button(text=operation, bd = 5, font = ('Arial', 15), fg='red', command=CE)

win = tk.Tk()
win.geometry(f'240x360+100+200')
win['bg'] = '#66A3D2'
...
make_operation_button('.').grid(row=5, column=2, stick='wens', padx=5, pady=5)
